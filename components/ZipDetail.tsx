...
interface ZipDetailProps {
  zipCode: string;
  countyName: string;
  zipBusinesses: Business[];
  zipTotalEmployees: number;
  zipTotalRevenue: string;
  onClose: () => void;
  onSelectCounty: (county: string) => void;
  onSelectBusiness: (b: Business) => void;
}

const ZipDetail: React.FC<ZipDetailProps> = ({ 
  zipCode,
  countyName,
  zipBusinesses, 
  zipTotalEmployees,
  ...
  return (
    <div className="absolute top-0 right-0 h-full w-full md:w-96 bg-slate-900/95 backdrop-blur-md border-l border-slate-700 shadow-2xl z-40 flex flex-col transition-transform duration-300 transform">
      ...
      <div className="p-6 border-b border-slate-700 flex justify-between items-start">
        <div>
          <h2 className="text-2xl font-bold text-white mb-1">Zip {zipCode}</h2>
          <span className="inline-block px-2 py-0.5 rounded text-xs font-mono bg-teal-900 text-teal-300 border border-teal-700">
            {zipBusinesses.length} Businesses
          </span>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="bg-slate-800 p-3 rounded border border-slate-700">
            <div className="flex items-center text-slate-400 text-xs uppercase font-bold mb-1">
              <Users size={14} className="mr-1" /> Zip Employees
            </div>
            <div className="text-xl font-semibold text-white">{zipTotalEmployees.toLocaleString()}</div>
          </div>
          <div className="bg-slate-800 p-3 rounded border border-slate-700">
            <div className="flex items-center text-slate-400 text-xs uppercase font-bold mb-1">
              <DollarSign size={14} className="mr-1" /> Zip Revenue
            </div>
            <div className="text-xl font-semibold text-emerald-400">{zipTotalRevenue}</div>
          </div>
        </div>
        ...
        <div className="space-y-3">
          ...
          <div className="flex flex-col space-y-2 select-none">
            ...
            <div 
                ...
                title={`Go to ${countyName} County`}
            >
              <div className="w-2 h-2 rounded-full bg-orange-500"></div>
              <span>{countyName}</span>
            </div>
            
            <div className="flex items-center space-x-2 text-white ml-12 border-l border-slate-700 pl-4 font-bold">
        ...
        {/* Business List */}
        <div>
           <h3 className="text-sm font-bold text-slate-400 uppercase tracking-wider mb-3">Businesses in {zipCode}</h3>
           <div className="space-y-3">
             {zipBusinesses.length === 0 ? (
               <div className="text-slate-500 text-sm italic">No businesses found in this Zip Code.</div>
             ) : (
               zipBusinesses.map(b => (
                 <div 
                    key={b.id} 
